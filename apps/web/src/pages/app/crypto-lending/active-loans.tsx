...
const ActiveLoansContent: React.FC = () => {
  ...
  const renderContent = () => {
    ...
    if (pagination.total === 0) {
      return (
        <EmptyState icon={NoSymbolIcon} label="No active loans available" />
      );
    }
...
const ActiveLoans: React.FC = () => {
  return (
    <LoansProvider status={NftFiLoanStatus.Active}>
      <AppLayout pageTitle="Crypto Lending - Active Loans">
        <ActiveLoansContent />
      </AppLayout>
    </LoansProvider>
  );
